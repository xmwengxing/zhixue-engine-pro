 ...
 * 年级值到标签的映射
 */
export const GRADE_VALUE_TO_LABEL = new Map(
 ...
 * 年级标签到值的映射
 */
export const GRADE_LABEL_TO_VALUE = new Map(
  GRADE_OPTIONS.map(g => [g.label, g.value])
);
...
export function isValidGrade(grade: string): boolean {
  return GRADE_VALUE_TO_LABEL.has(grade);
}

/**
 * 获取年级标签
 */
export function getGradeLabel(value: string): string | undefined {
  return GRADE_VALUE_TO_LABEL.get(value);
}

/**
 * 获取年级值
 */
export function getGradeValue(label: string): string | undefined {
  return GRADE_LABEL_TO_VALUE.get(label);
}
